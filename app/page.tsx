...
type Tab = 'build' | 'generate' | 'preview' | 'library';
    ...
    <main className="h-screen flex flex-col overflow-hidden bg-[#0a0a0f]">
      ...
      <header className="shrink-0 border-b border-white/10 px-5 py-2.5 flex items-center gap-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold text-white leading-tight tracking-wide">
              FDR Assessment Creator
            </h1>
            <p className="text-[10px] text-gray-500 leading-tight">
              Key Assessment builder · Math 9
            </p>
          </div>
        </div>
        ...
        <div className="ml-auto text-xs text-gray-600">
          {assessment.meta.course
            ? `${assessment.meta.course}${
                assessment.meta.block ? ` · Block ${assessment.meta.block}` : ''
              }`
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {tab === 'generate' && (
          <GenerateTab
            assessment={assessment}
            onLatexReady={setLatex}
            onGoToPreview={() => setTab('preview')}
          />
        )}
        {tab === 'preview' && (
          <PreviewTab latex={latex} />
        )}
        {tab === 'library' && (
          <LibraryTab
